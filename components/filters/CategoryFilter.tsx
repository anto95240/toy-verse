  ...
  return (
    <div className="pb-2">
      <div className="flex items-center justify-between mb-4" onClick={() => setIsCollapsed(!isCollapsed)}>
        ...
        <button
          className="text-text-prim hover:text-blue-600 transition-colors p-1"
          aria-label={isCollapsed ? "Fermer les catégories" : "Ouvrir les catégories"}
        >
          <FontAwesomeIcon 
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isCollapsed ? 'max-h-0 opacity-0' : 'max-h-[1000px] opacity-100'}`}>
        ...
        {hasMore && !searchTerm && (
          <button
            ...
            className="mx-auto text-sm text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-blue-50"
          >
