  ...
  const Section = ({ id, icon, title, children }: SectionProps) => (
    ...
    >
      ...
      >
        ...
        <FontAwesomeIcon
          icon={expanded === id ? faChevronUp : faChevronDown}
          className="text-muted-foreground text-sm"
        />
      </button>
      <div
        className={`overflow-hidden transition-all duration-300 ${
          expanded === id ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"
        }`}
      >
        {children}
      </div>
  ...
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm divide-y divide-border">
        ...
        >
          ...
          <div className="p-4 bg-background/50 rounded-xl border border-border/50 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <div className="text-[10px] font-bold flex gap-2">
                {!validations.emailsMatch && form.confirmEmail && (
                  <span className="text-destructive">
                    <FontAwesomeIcon icon={faExclamationCircle} /> Différents
                  </span>
                )}
                {form.email && !validations.isValidDomain && (
                  <span className="text-orange-500">
                    <FontAwesomeIcon icon={faExclamationCircle} /> Domaine
                    invalide
                  </span>
                )}
              </div>
            ...
            <FormInput
              ...
              onChange={(v) => updateForm("confirmEmail", v)}
            />
          </div>
          <div className="flex justify-end gap-2">
            <button
              ...
              disabled={
                loading ||
                !validations.emailsMatch ||
                !validations.isValidDomain
              }
              className="px-6 py-2 bg-primary text-primary-foreground rounded-lg font-bold disabled:opacity-50"
            >
              Enregistrer
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {AVAILABLE_STATS.map((s) => (
              <button
                ...
                className={`flex justify-between p-3 rounded-xl border transition-all ${
      ...
      <Section id="pass" icon={faLock} title="Mot de passe">
        ...
        >
          <PasswordInput
            ...
            label="Nouveau"
            value={pass.new}
            onChange={(v) => updatePass("new", v)}
          />
          <PasswordInput
            id="pass2"
            label="Confirmer"
            value={pass.confirm}
            onChange={(v) => updatePass("confirm", v)}
          />
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={loading || !validations.passwordsMatch}
              className="px-6 py-2 bg-primary text-primary-foreground rounded-lg font-bold disabled:opacity-50"
            >
              Mettre à jour
            </button>
      ...
      <button
        ...
        className="w-full p-5 text-destructive hover:bg-destructive/10 text-left flex items-center gap-4 font-medium transition-colors"
      >
