...
export function calculateProfileStats(toys: Toy[], themes: Theme[]) {
  ...
  const getTop = (key: keyof Toy | 'theme_id', map?: Map<string, string>) => {
    ...
    toys.forEach(t => {
       ...
       const val = key === 'theme_id' ? (map?.get(t.theme_id) || "Inconnu") : (t[key] as any)
  ...
  const sortedToys = [...toys].sort((a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime())
  const lastToy = sortedToys[0]
  
  const releaseYears = toys.map(t => t.release_date).filter((d): d is number => typeof d === 'number' && d > 1900)
  ...
  return {
    ...
    topCategory, 
    favoriteBrand,
    lastToy: { name: lastToy?.nom || "-", theme: lastToy ? (themeMap.get(lastToy.theme_id) || "") : "" },
    oldestYear, 
    recentAdditions
  }
}