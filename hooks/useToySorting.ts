...
export type SortCriteria = 'added_desc' | 'added_asc' | 'release_desc' | 'release_asc'

export function useToySorting(toys: Toy[], searchResults: (Toy & { theme_name: string })[], isSearchActive: boolean, themeId: string) {
  const [itemsPerPage, setItemsPerPage] = useState(24)
  const [currentPage, setCurrentPage] = useState(1)
  const [sortCriteria, setSortCriteria] = useState<SortCriteria>('added_desc')

  // Déterminer la liste source
  ...
  const sortedToys = useMemo(() => {
    ...
    sorted.sort((a, b) => {
       switch (sortCriteria) {
          case 'added_desc': return (new Date(b.created_at || 0).getTime()) - (new Date(a.created_at || 0).getTime())
          case 'added_asc': return (new Date(a.created_at || 0).getTime()) - (new Date(b.created_at || 0).getTime())
          case 'release_desc':
  ...
  const paginatedToys = useMemo(() => {
    return sortedToys.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
