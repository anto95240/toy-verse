...
export function useProfileLogic(user: any) {
  ...
  const updatePass = (k: keyof typeof pass, v: string) => setPass(p => ({ ...p, [k]: v }))

  // Validations
  const emailsMatch = form.email === form.confirmEmail && form.email !== ""
  const isValidDomain = form.email.includes('@') && ALLOWED_DOMAINS.includes(form.email.split('@')[1]?.toLowerCase())
  ...
  const updateProfile = async () => {
    ...
    setLoading(true)
    try {
      await updateUserProfile({ firstName: form.firstName, lastName: form.lastName, email: form.email.trim() })
  ...
  const saveStats = async () => {
    ...
    if (!error) { showToast("Préférences sauvegardées", "success"); router.refresh() } 
    else { showToast(error.message, "error") }
    setLoading(false)
  }
  ...
  return {
    form, updateForm,
    pass, updatePass,
    selectedStats, toggleStat, saveStats,
    loading,
