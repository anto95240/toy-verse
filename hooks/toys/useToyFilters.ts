...
export function useToyFilters(themeId: string, sessionExists: boolean) {
  ...
  useEffect(() => {
    ...
    const loadCategories = () =>
      (supabase.from("toys").select("categorie") as any)
        .eq("theme_id", themeId)
  ...
  useEffect(() => {
    if (!sessionExists) return;
    const loadStudios = () =>
      (supabase.from("toys").select("studio") as any)
        .eq("theme_id", themeId)
        .not("studio", "is", null)
  ...
  useEffect(() => {
    if (!sessionExists) return;
    const loadYears = () =>
      (supabase.from("toys").select("release_date") as any)
        .eq("theme_id", themeId)
    ...
    (oldToy: Toy | null, newToy: Toy) => {
      ...
      setFilterCounts((prevCounts) => {
        ...
        if (oldToy) {
          ...
          if (oldToy.is_soon && newCounts.soon["true"])
            newCounts.soon["true"]--;
        } else {
          newCounts.totalToys++;
        }
  ...
  const handleNbPiecesChange = useCallback(
    (range: string) =>
  ...
  const handleSoonChange = useCallback(
    (value: boolean | null) =>
      setFilters((prev) => ({ ...prev, isSoon: value })),
    []
  );
  const handleReleaseYearChange = useCallback(
    (year: string) => setFilters((prev) => ({ ...prev, releaseYear: year })),
    []
  );
